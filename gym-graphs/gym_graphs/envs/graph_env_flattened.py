...
class GraphEnvF(gym.Env):
    ...
    def __init__(self, N, mode='random', model=None, reward_weights=(0, 0)):
        ...
        # instead of doing two n^2 \times n^2 action spaces, code tuples (s, t)
        # as 0 -> (0, 0), 1 -> (0, 1), 2 -> (0, 2), ..., n^2 -> (1, 0), etc.
        # thus, from_idx = a // n^2; to_idx = a % n^2
        # this requires n^4 entries. probably not ideal, but it's the same *size* as before. 
        self.action_space = spaces.Discrete(self.N ** 4)
    ...
    def action_to_nodes(self, action):
        source_idx = action // (self.N ** 2)
        target_idx = action % (self.N ** 2)
